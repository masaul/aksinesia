...
$idView = $_GET["id"];
$result = mysqli_query($databaseConn, "SELECT * FROM tbl_contact WHERE id_pesan = $idView");
          ...
          <div class="container mt-3">
            ...
            <?php foreach ($result as $dataDbs) : ?>
                ...
                <div class="nama mt-2">
                    ...
                    <input type="text" id="nama" class="form-control" value="<?= $dataDbs["nama"]; ?>" disabled>
                </div>
                <div class="email mt-2">
                    <label for="email" class="fw-bold mb-1">Email :</label>
                    <input type="text" class="form-control" id="email" value="<?= $dataDbs["email"]; ?>" disabled>
                </div>
                <div class="nohp mt-2">
                    <label for="nohp" class="fw-bold mb-1">No Handphone :</label>
                    <input type="text" class="form-control" id="nohp" value="<?= $dataDbs["nohp"]; ?>" disabled>
                </div>
                <div class="pesan mt-2">
                    <label for="pesan" class="fw-bold mb-1">Pesan :</label>
                    <textarea class="form-control" name="pesan" id="pesan" cols="30" rows="3" disabled><?= $dataDbs["pesan"]; ?></textarea>
                </div>
